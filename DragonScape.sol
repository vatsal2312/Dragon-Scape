...
// File: contracts/DSCAPEDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/DSCAPE.sol

pragma solidity ^0.7.6;

contract DSCAPE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DSCAPEDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("DSCAPE", "DSCAPE") {
        // LP burn address
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "DSCAPE: The dividend tracker already has that address");

        DSCAPEDividendTracker newDividendTracker = DSCAPEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DSCAPE: The new dividend tracker must be owned by the DSCAPE token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DSCAPE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "DSCAPE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "DSCAPE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DSCAPE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DSCAPE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DSCAPE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DSCAPE: This account cannot send tokens until trading is enabled");
        }
        if(amount == 0) {
...
contract DSCAPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DSCAPE_Dividend_Tracker", "DSCAPE_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DSCAPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DSCAPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DSCAPE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DSCAPE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DSCAPE_Dividend_Tracker: Cannot update claimWait to same value");
